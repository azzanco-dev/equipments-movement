...
export function AdminProjects() {
  ...
  function openEdit(p: Project) {
    setEditing(p); setNameAr(p.name_ar); setNameEn(p.name_en); setFormError(null); setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="card text-center py-12"><p className="text-muted">{t('noProjects')}</p></div>
      ) : (
      ...
      {/* Import Modal */}
      <Modal open={importModalOpen} onClose={() => setImportModalOpen(false)} title={t('importProjects')} size="xl">
        ...
        {importData.length === 0 && !importResult ? (
          <div className="space-y-4">
            <div className="flex justify-end">
              <button onClick={() => downloadProjectTemplate(t)} className="btn-ghost text-sm"><Download size={16} /> {t('downloadTemplate')}</button>
            </div>
            <div
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-surface">
                  <tr className="border-b" style={{ borderColor: 'var(--border)' }}>
                    ...
                    <th className="table-header text-start px-3 py-2">{t('projectNameAr')}</th>
                    <th className="table-header text-start px-3 py-2">{t('projectNameEn')}</th>
