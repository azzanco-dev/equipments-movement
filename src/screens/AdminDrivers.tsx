...
export function AdminDrivers({ onSelectDriver }: { onSelectDriver: (id: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <DataListToolbar compact config={driversListConfig} search={list.searchInput} onSearch={list.setSearchInput} sort={list.sort} direction={list.direction} onSort={list.setSort} pageSize={list.pageSize} onPageSize={list.setPageSize} filters={list.filters} onFilters={list.setFilters} selectedCount={selection.selected.size} bulkActions={<button className="btn-outline h-[34px] px-3 py-0 text-sm" onClick={removeSelected}><Trash2 size={15} />{t('delete')}</button>} actions={<><button className="btn-outline h-[34px] px-3 py-0 text-sm" onClick={() => setImportOpen(true)}><FileSpreadsheet size={15} />استيراد Excel</button><button className="btn-primary h-[34px] px-3 py-0 text-sm" onClick={openCreate}><Plus size={15} />{t('addDriver')}</button></>} />
