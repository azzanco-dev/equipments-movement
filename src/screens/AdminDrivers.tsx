...
export function AdminDrivers({
  ...
  const save = async () => {
    if (
      !form.full_name.trim() ||
      (form.id_number && !/^\d{5,20}$/.test(form.id_number)) ||
      (form.mobile_number && !/^\+?\d{7,15}$/.test(form.mobile_number))
    ) {
      setError(t('driverValidationError'))
      return
    }
    setSaving(true)
    setError(null)
  ...
  const remove = async (driver: Driver) => {
    ...
    const { error: deleteError } = await supabase
      ...
      .delete()
      .eq('id', driver.id)
    if (deleteError) setError(t('driverDeleteBlocked'))
    else fetchDrivers()
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <DataListToolbar
        ...
        bulkActions={
          <button
            className="btn-outline h-8 px-3 py-0 text-[13px]"
                    ...
                    <input
                      ...
                      checked={
                        ...
                        drivers.every((driver) =>
                          selection.selected.has(driver.id),
                        )
                      }
                      onChange={() =>
                        selection.togglePage(drivers.map((driver) => driver.id))
                      }
                    />
                  </th>
                ...
                {drivers.map((driver) => (
                  ...
                  >
                    <td className="px-3 py-3">
                      <input
                        type="checkbox"
                        checked={selection.selected.has(driver.id)}
            ...
            <div>
              ...
              <input
                ...
                onChange={(event) =>
                  setForm({
                    ...form,
                    id_number: event.target.value.replace(/\D/g, ''),
                  })
                }
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Select
                ...
                options={[
                  { value: '', label: '—' },
                  ...DRIVER_NATIONALITIES.map((value) => ({
                    value,
                    label: value,
                  })),
                ]}
            ...
            <div>
              ...
              <Select
                ...
                options={[
                  { value: '', label: '—' },
                  ...DRIVER_EMPLOYMENT_TYPES.map((value) => ({
                    value,
                    label: value,
                  })),
                ]}
