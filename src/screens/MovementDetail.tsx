    ...
    <div className="space-y-6">
      ...
      {driverEntryId && log.movement_context !== 'workshop' && (
        <div className="card space-y-4">
          ...
          {driverChanges.length === 0 ? <p className="text-sm text-muted">{t('noDriverChanges')}</p> : <div className="space-y-2">{driverChanges.map((change) => <div key={change.id} className="rounded-lg border px-3 py-2 text-sm" style={{ borderColor: 'var(--border)' }}><div className="grid gap-1 sm:grid-cols-2"><p><span className="text-muted">{t('previousDriver')}:</span> <span className="font-medium" dir="auto">{change.previous_driver_name}</span></p><p><span className="text-muted">{t('newDriver')}:</span> <span className="font-medium" dir="auto">{change.new_driver_name}</span></p></div><p className="mt-1 text-xs text-muted">{formatDateTime(change.changed_at)}{change.changer?.full_name ? ` — ${change.changer.full_name}` : ''}</p>{change.note && <p className="mt-1 text-xs">{change.note}</p>}</div>)}</div>}
        </div>
      )}
