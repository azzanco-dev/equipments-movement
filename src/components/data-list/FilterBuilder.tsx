...
export function FilterBuilder({ fields, filters, onChange, compact = false }: { fields: FilterField[]; filters: ListFilter[]; onChange: (filters: ListFilter[]) => void; compact?: boolean }) {
  ...
  const labels: Record<FilterOperator, string> = lang === 'ar'
    ? { eq: '=', neq: '≠', in: 'ضمن', not_in: 'ليس ضمن', like: 'يحتوي', not_like: 'لا يحتوي', is_set: 'له قيمة', is_not_set: 'بلا قيمة', gt: '>', lt: '<', gte: '≥', lte: '≤', between: 'بين' }
    : { eq: '=', neq: '≠', in: 'In', not_in: 'Not in', like: 'Contains', not_like: 'Does not contain', is_set: 'Is set', is_not_set: 'Is not set', gt: '>', lt: '<', gte: '≥', lte: '≤', between: 'Between' };
