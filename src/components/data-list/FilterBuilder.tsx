...
export function FilterBuilder({ fields, filters, onChange }: { fields: FilterField[]; filters: ListFilter[]; onChange: (filters: ListFilter[]) => void }) {
  ...
  return <div className="space-y-2">
    {filters.map((filter) => { const field = fields.find((item) => item.key === filter.field) ?? fields[0]; const noValue = filter.operator === 'is_set' || filter.operator === 'is_not_set'; return <div key={filter.id} className="grid gap-2 sm:grid-cols-[1fr_140px_1fr_auto]">
      <Select value={filter.field} onChange={(value) => { const next = fields.find((item) => item.key === value) ?? fields[0]; patch(filter.id, { field: value, operator: next.operators[0], value: '', valueTo: '' }); }} options={fields.map((item) => ({ value: item.key, label: item.label }))} />
      <Select value={filter.operator} onChange={(value) => patch(filter.id, { operator: value as FilterOperator })} options={field.operators.map((operator) => ({ value: operator, label: labels[operator] }))} />
      {noValue ? <div /> : field.options ? <Select value={filter.value} onChange={(value) => patch(filter.id, { value })} options={[{ value: '', label: '—' }, ...field.options]} /> : <div className="flex gap-2"><input className="input" type={field.type === 'date' ? 'date' : field.type === 'number' ? 'number' : 'text'} value={filter.value} onChange={(event) => patch(filter.id, { value: event.target.value })} />{filter.operator === 'between' && <input className="input" type={field.type === 'date' ? 'date' : 'number'} value={filter.valueTo ?? ''} onChange={(event) => patch(filter.id, { valueTo: event.target.value })} />}</div>}
