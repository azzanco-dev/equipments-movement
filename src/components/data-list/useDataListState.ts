...
export function useDataListState(config: DataListConfig, prefix = '') {
  ...
  const direction = params.get(key('dir')) === 'desc' ? 'desc' : params.get(key('dir')) === 'asc' ? 'asc' : (config.defaultDirection ?? 'asc');
  const filters = useMemo<ListFilter[]>(() => {
    try {
      const value = JSON.parse(params.get(key('filters')) ?? '[]') as ListFilter[];
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => { if (searchInput !== search) update({ [key('q')]: searchInput.trim(), [key('page')]: 1 }); }, 300);
  ...
  return {
    ...
    setPageSize: (value: number) => update({ [key('size')]: value, [key('page')]: 1 }),
    setSort: (field: string, dir: 'asc' | 'desc') => update({ [key('sort')]: field, [key('dir')]: dir, [key('page')]: 1 }),
    setFilters: (value: ListFilter[]) => update({ [key('filters')]: value.length ? JSON.stringify(value) : null, [key('page')]: 1 }),
