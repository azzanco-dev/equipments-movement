...
export function DataListToolbar({ config, search, onSearch, sort, direction, onSort, pageSize, onPageSize, filters, onFilters, selectedCount = 0, bulkActions, actions, menuActions, primaryAction, compact = false }: ToolbarProps) {
  ...
  useEffect(() => { const close = (event: MouseEvent) => { const target = event.target as Element; if (target.closest?.('[data-select-portal="true"], [data-list-popover="true"], [data-list-trigger="true"]')) return; setOpen(null); }; document.addEventListener('mousedown', close); return () => document.removeEventListener('mousedown', close); }, []);
  ...
  return <div className="space-y-3" ref={toolbarRef}>
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="relative">
        <button data-list-trigger="true" className="btn-outline" onClick={() => toggle('filters')}><Filter size={15} />{t('filters')}{filters.length > 0 && <span className="rounded bg-gray-100 px-1.5 text-xs dark:bg-gray-700">{filters.length}</span>}<ChevronDown size={14} /></button>
        {open === 'filters' && <div data-list-popover="true" className="absolute end-0 top-[calc(100%+6px)] z-40 w-[min(560px,calc(100vw-24px))] rounded-lg border p-3 shadow-xl" style={{ background: 'var(--bg)', borderColor: 'var(--border)' }}>
      ...
      <div className="relative">
        <button data-list-trigger="true" className="btn-outline" onClick={() => toggle('sort')}>{direction === 'asc' ? <ArrowUpAZ size={15} /> : <ArrowDownAZ size={15} />}<span className="hidden sm:inline">{currentSort?.label ?? t('sortBy')}</span><ChevronDown size={14} /></button>
        {open === 'sort' && <div data-list-popover="true" className="absolute end-0 top-[calc(100%+8px)] z-40 min-w-[230px] rounded-xl border py-2 shadow-xl" style={{ background: 'var(--bg)', borderColor: 'var(--border)' }}>
      ...
      {menuActions && <div className="relative"><button data-list-trigger="true" className="btn-outline px-2.5" onClick={() => toggle('actions')} aria-label={t('moreActions')}><MoreHorizontal size={18} /></button>{open === 'actions' && <div data-list-popover="true" className="absolute end-0 top-[calc(100%+8px)] z-40 min-w-[190px] space-y-1 rounded-xl border p-2 shadow-xl [&>button]:w-full [&>button]:justify-start" style={{ background: 'var(--bg)', borderColor: 'var(--border)' }} onClick={() => setOpen(null)}>{menuActions}</div>}</div>}
