...
export function TimePicker({
  ...
  const selectedHour = String(Number(selected24Hour) % 12 || 12).padStart(
    2,
    '0',
  )
  ...
  const isAfterMax = (hour24: string, minute: string) =>
    Boolean(max && `${hour24}:${minute}` > max)

  const selectHour = (hour: string) => {
    const hour24 = to24Hour(hour, selectedPeriod)
    if (!isAfterMax(hour24, selectedMinute))
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      {open && (
        ...
        >
          <div
            className="grid grid-cols-3 border-b px-3 py-2 text-center text-xs font-medium text-muted"
            style={{ borderColor: 'var(--border)' }}
          >
          ...
          <div className="grid grid-cols-3" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            <div className="max-h-52 overflow-y-auto p-1">
              {HOURS.map((hour) => {
                const disabled = isAfterMax(
                  to24Hour(hour, selectedPeriod),
                  selectedMinute,
                )
                return (
                  <button
            ...
            <div
              className="max-h-52 overflow-y-auto border-s p-1"
              style={{ borderColor: 'var(--border)' }}
            >
              {MINUTES.map((minute) => {
            ...
            <div
              className="border-s p-1"
              style={{ borderColor: 'var(--border)' }}
            >
              {(['AM', 'PM'] as const).map((period) => {
                const disabled = isAfterMax(
                  to24Hour(selectedHour, period),
                  selectedMinute,
                )
