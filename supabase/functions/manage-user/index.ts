...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (body.action === 'change_own_password') {
      ...
      if (passwordError)
        return json({ error: 'Could not update password' }, 400)
      const { error: profileError } = await adminClient
        .from('profiles')
        .update({ must_change_password: false })
        .eq('id', user.id)
      if (profileError)
        return json({ error: 'Could not complete password change' }, 500)
    ...
    if (body.action === 'update') {
      ...
      if (authError)
        return json({ error: 'Could not update login details' }, 400)
