-- Workshop exits must only list equipment whose latest workshop movement is an entry.
  ...
  WHERE e.is_active
    ...
    AND (
      ...
      OR (
        ...
        AND (
          v_role IN ('admin', 'workshop_manager', 'assistant_workshop_manager')
          OR last_workshop_movement.supervisor_id = auth.uid()
        )
      )
    )
  ORDER BY e.code
