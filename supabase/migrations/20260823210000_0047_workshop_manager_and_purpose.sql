-- Workshop manager role and manager-controlled classification of workshop entries.

ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_role_check;
ALTER TABLE public.profiles ADD CONSTRAINT profiles_role_check
  CHECK (role IN ('admin', 'supervisor', 'workshop', 'workshop_manager'));

ALTER TABLE public.entry_exit_logs ADD COLUMN IF NOT EXISTS workshop_purpose text;
ALTER TABLE public.entry_exit_logs ADD COLUMN IF NOT EXISTS previous_operational_status text;
ALTER TABLE public.entry_exit_logs DROP CONSTRAINT IF EXISTS entry_exit_logs_workshop_purpose_check;
ALTER TABLE public.entry_exit_logs ADD CONSTRAINT entry_exit_logs_workshop_purpose_check
  CHECK (workshop_purpose IS NULL OR (movement_context='workshop' AND movement_type='entry' AND workshop_purpose IN ('maintenance','parking')));
ALTER TABLE public.entry_exit_logs DROP CONSTRAINT IF EXISTS entry_exit_logs_previous_operational_status_check;
ALTER TABLE public.entry_exit_logs ADD CONSTRAINT entry_exit_logs_previous_operational_status_check
  CHECK (previous_operational_status IS NULL OR previous_operational_status IN ('operational','maintenance','stopped'));
...
BEGIN
  IF NEW.raw_user_meta_data->>'admin_created' = 'true'
     AND NEW.raw_user_meta_data->>'role' IN ('admin', 'supervisor', 'workshop', 'workshop_manager') THEN
...
BEGIN
  ...
  IF p_role NOT IN ('admin', 'supervisor', 'workshop', 'workshop_manager') THEN RAISE EXCEPTION 'invalid role'; END IF;
...
USING (
  ...
  OR (public.current_user_role()='workshop_manager' AND role IN ('workshop','workshop_manager'))
);

DROP POLICY IF EXISTS "select_entry_exit_logs" ON public.entry_exit_logs;
CREATE POLICY "select_entry_exit_logs" ON public.entry_exit_logs FOR SELECT TO authenticated
USING (
  supervisor_id=auth.uid() OR public.is_admin()
  OR (public.current_user_role()='workshop_manager' AND movement_context='workshop')
);
  ...
  SELECT EXISTS (
    ...
    WHERE l.id=p_movement_id
      AND (l.supervisor_id=auth.uid() OR public.is_admin()
        OR (public.current_user_role()='workshop_manager' AND l.movement_context='workshop'))
  );
$$;
...
BEGIN
  ...
  IF v_role NOT IN ('admin', 'workshop', 'workshop_manager') OR p_numbering_status NOT IN ('numbered', 'unnumbered') OR v_plate = '' THEN
...
BEGIN
  ...
  IF v_role NOT IN ('admin','workshop','workshop_manager') OR p_movement_type NOT IN ('entry','exit') THEN RAISE EXCEPTION 'workshop role required'; END IF;
  RETURN QUERY
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE l.equipment_id=e.id ORDER BY l.recorded_at DESC,l.id DESC LIMIT 1
...
BEGIN
  SELECT role INTO v_role FROM public.profiles WHERE id=auth.uid();
  IF v_role NOT IN ('admin','workshop_manager') THEN RAISE EXCEPTION 'workshop manager required'; END IF;
  IF p_purpose NOT IN ('maintenance','parking') THEN RAISE EXCEPTION 'invalid workshop purpose'; END IF;
  SELECT * INTO v_entry FROM public.entry_exit_logs
    WHERE id=p_entry_log_id AND movement_context='workshop' AND movement_type='entry' FOR UPDATE;
  ...
  IF NOT v_is_open THEN
    UPDATE public.entry_exit_logs SET workshop_purpose=p_purpose,previous_operational_status=NULL WHERE id=v_entry.id;
    RETURN;
  END IF;
  ...
  ELSE
    UPDATE public.entry_exit_logs SET workshop_purpose=p_purpose WHERE id=v_entry.id;
  END IF;
END; $$;
...
BEGIN
  ...
  IF NEW.movement_context='workshop' THEN
    IF v_role NOT IN ('admin','workshop','workshop_manager') THEN RAISE EXCEPTION 'workshop role required'; END IF;
