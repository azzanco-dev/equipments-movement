-- Aggregates entry reports in PostgreSQL so the client never loads a full movement table.
CREATE OR REPLACE FUNCTION public.get_entry_report_summary(p_from timestamptz, p_to timestamptz)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY INVOKER
...
AS $$
WITH entries AS (
  SELECT l.*, e.code equipment_code, e.type equipment_type,
    e.plate_number, c.name_ar company_name_ar, c.name_en company_name_en,
    p.name_ar project_name_ar, p.name_en project_name_en,
  ...
  WHERE l.movement_context='site' AND l.movement_type='entry'
  ...
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'driver_name', COALESCE(last_driver_name, driver_name),
    'entry_recorded_at', entry_recorded_at
  ) ORDER BY entry_recorded_at DESC) value
  FROM (SELECT * FROM public.equipment_visits WHERE movement_context='site' AND exit_log_id IS NULL ORDER BY entry_recorded_at DESC LIMIT 5) v
), latest AS (
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'equipment',jsonb_build_object('id',equipment_id,'code',equipment_code,'type',equipment_type,'plate_number',plate_number),
    'company',CASE WHEN company_id IS NULL THEN NULL ELSE jsonb_build_object('id',company_id,'name_ar',company_name_ar,'name_en',company_name_en) END,
    'project',CASE WHEN project_id IS NULL THEN NULL ELSE jsonb_build_object('id',project_id,'name_ar',project_name_ar,'name_en',project_name_en) END,
    'supervisor',jsonb_build_object('id',supervisor_id,'full_name',supervisor_name),
    'driver',CASE WHEN driver_id IS NULL THEN NULL ELSE jsonb_build_object('id',driver_id,'mobile_number',driver_mobile) END
  ) ORDER BY recorded_at DESC, id DESC) value FROM (SELECT * FROM entries ORDER BY recorded_at DESC,id DESC LIMIT 10) x
)
