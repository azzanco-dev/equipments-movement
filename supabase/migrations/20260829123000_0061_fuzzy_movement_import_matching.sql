...
BEGIN
  ...
  LOOP
    ...
    SELECT c.id, c.name_ar, c.name_en,
           greatest(
             similarity(lower(COALESCE(c.name_ar, '')), lower(COALESCE(v_value, ''))),
             similarity(lower(COALESCE(c.name_en, '')), lower(COALESCE(v_value, '')))
           ) AS match_score
    INTO v_company
    ...
    ORDER BY
      CASE
        WHEN lower(btrim(c.name_ar)) = lower(v_value)
          OR lower(btrim(c.name_en)) = lower(v_value) THEN 0
        ELSE 1
      END,
      greatest(
        similarity(lower(COALESCE(c.name_ar, '')), lower(v_value)),
        similarity(lower(COALESCE(c.name_en, '')), lower(v_value))
      ) DESC,
      c.created_at,
      c.id
    LIMIT 1;
    ...
    SELECT p.id, p.name_ar, p.name_en,
           greatest(
             similarity(lower(COALESCE(p.name_ar, '')), lower(COALESCE(v_value, ''))),
             similarity(lower(COALESCE(p.name_en, '')), lower(COALESCE(v_value, '')))
           ) AS match_score
    INTO v_project
    ...
    ORDER BY
      CASE WHEN EXISTS (
        ...
        WHERE cp.company_id = v_company.id AND cp.project_id = p.id
      ) THEN 0 ELSE 1 END,
      CASE
        WHEN lower(btrim(p.name_ar)) = lower(v_value)
          OR lower(btrim(p.name_en)) = lower(v_value) THEN 0
        ELSE 1
      END,
      greatest(
        similarity(lower(COALESCE(p.name_ar, '')), lower(v_value)),
        similarity(lower(COALESCE(p.name_en, '')), lower(v_value))
      ) DESC,
      p.created_at,
      p.id
    LIMIT 1;
    ...
    v_result := v_result || jsonb_build_array(
      v_row || jsonb_build_object(
        ...
        'company_match_score', v_company.match_score,
        'project_id', v_project.id,
        'project_label', COALESCE(v_project.name_ar, v_project.name_en),
        'project_match_score', v_project.match_score,
