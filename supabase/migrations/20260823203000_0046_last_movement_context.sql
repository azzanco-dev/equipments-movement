-- Return the movement context so the UI can distinguish site and workshop status.
...
RETURNS TABLE(movement_type text, movement_context text, recorded_at timestamptz, supervisor_id uuid, company_id uuid, project_id uuid, contractor_equipment_code text, driver_id uuid, driver_name text)
